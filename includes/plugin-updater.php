...
if ( ! defined( 'ABSPATH' ) || class_exists( 'Clinical_Anti_Spam_Updater' ) )
	return;
 ...
 */
class Clinical_Memcachier_Updater{
	...
	public function plugins_api_result( $res, $action, $args ) {
		...
		if ( $plugin_slug == $updater_data['slug'] && $action == 'plugin_information' ){
			...
			if ( is_wp_error( $request ) ) {
				$res = new WP_Error( 'plugins_api_failed', '<p>' . __( 'An Unexpected HTTP Error occurred during the API request.', 'Clinical-Anti-Spam' ) . '</p><p><a href="?" onclick="document.location.reload(); return false;">' . __( 'Try again', 'Clinical-Anti-Spam' ) . '</a></p>', $request->get_error_message() );
			}
			...
			/* If no error, construct the data */
				...
				/* If data is empty or not an object */
					...
					$res = new WP_Error( 'plugins_api_failed', __( 'An unknown error occurred', 'Clinical-Anti-Spam' ), wp_remote_retrieve_body( $request ) );
	...
	public function upgrader_post_install( $true, $hook_extra, $result ) {
		...
		if ( isset( $hook_extra ) ){
			...
			if ( isset( $hook_extra['plugin'] ) && $hook_extra['plugin'] == $plugin_base ){
				...
				/* Update message */
				$fail = __( 'The plugin has been updated, but could not be reactivated. Please reactivate it manually.', 'Clinical-Anti-Spam' );
				$success = __( 'Plugin reactivated successfully. ', 'Clinical-Anti-Spam' );
	...
	public function add_dashboard_widget() {
		...
		/* Widget name */
		$widget_name = $updater_data['name'] . __( ' Plugin Updates', 'Clinical-Anti-Spam' );
	...
	public function dashboard_widget_callback() {
		...
		if ( !empty( $widget_option ) && is_array( $widget_option ) ){
			...
			if ( true === $updater_data['role'] ){
				...
				echo '<p>'. __( 'Username: ', 'Clinical-Anti-Spam' ) . '<code>' . $username . '</code></p>';

				/* activation key input */
				$key = isset( $widget_option['key'] ) ? $widget_option['key'] : '' ;
				echo '<p>'. __( 'Email: ', 'Clinical-Anti-Spam' ) . '<code>' . $key . '</code></p>';
			}
			else{

				/* activation key input */
				$key = isset( $widget_option['key'] ) ? $widget_option['key'] : '' ;
				echo '<p>'. __( 'Key: ', 'Clinical-Anti-Spam' ) . '<code>' . $key . '</code></p>';
			}


			/* if key status is valid */
			if ( $widget_option['status'] == 'valid' ){
				_e( '<p>Your plugin update is <span style="color:green">active</span></p>', 'Clinical-Anti-Spam' );
			}
			/* if key is not valid */
			elseif( $widget_option['status'] == 'invalid' ){
				_e( '<p>Your input is <span style="color:red">not valid</span>, automatic updates is <span style="color:red">not active</span>.</p>', 'Clinical-Anti-Spam' );
				echo '<p><a href="' . $edit_url . '" class="button-primary">' . __( 'Edit Key', 'Clinical-Anti-Spam' ) . '</a></p>';
			}
			/* else */
			else{
				_e( '<p>Unable to validate update activation.</p>', 'Clinical-Anti-Spam' );
				echo '<p><a href="' . $edit_url . '" class="button-primary">' . __( 'Try again', 'Clinical-Anti-Spam' ) . '</a></p>';
			}
		}
		/* if activation key is not yet set/empty */
		else{
			echo '<p><a href="' . $edit_url . '" class="button-primary">' . __( 'Add Key', 'Clinical-Anti-Spam' ) . '</a></p>';
		}
	}
	...
	public function dashboard_widget_control_callback() {
		...
		<p>
			<label for="<?php echo $widget_id; ?>-username"><?php _e( 'User name', 'Clinical-Anti-Spam' ); ?></label>
		</p>
		<p>
			<input id="<?php echo $widget_id; ?>-username" name="<?php echo $widget_id; ?>[username]" type="text" value="<?php echo $username_option;?>"/>
		</p>
		<p>
			<label for="<?php echo $widget_id; ?>-key"><?php _e( 'Email', 'Clinical-Anti-Spam' ); ?></label>
		</p>
		<p>
		...
		<p>
			<label for="<?php echo $widget_id; ?>-key"><?php _e( 'Activation Key', 'Clinical-Anti-Spam' ); ?></label>
		</p>
		<p>
			<input id="<?php echo $widget_id; ?>-key" class="regular-text" name="<?php echo $widget_id; ?>[key]" type="text" value="<?php echo $key_option;?>"/>
		</p>
